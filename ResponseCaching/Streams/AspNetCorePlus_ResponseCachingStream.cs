...
internal sealed class AspNetCorePlus_ResponseCachingStream : Stream
{
    private readonly Stream _innerStream;
    private readonly long _maxBufferSize;
    private readonly int _segmentSize;
    private readonly AspNetCorePlus_SegmentWriteStream _segmentWriteStream;
    private readonly Action _startResponseCallback;

    internal AspNetCorePlus_ResponseCachingStream(Stream innerStream, long maxBufferSize, int segmentSize, Action startResponseCallback)
    {
        _innerStream = innerStream;
        _maxBufferSize = maxBufferSize;
        _segmentSize = segmentSize;
        _startResponseCallback = startResponseCallback;
        _segmentWriteStream = new AspNetCorePlus_SegmentWriteStream(_segmentSize);
    }
    ...
    internal AspNetCorePlus_CachedResponseBody GetCachedResponseBody()
    {
        if (!BufferingEnabled)
        {
            throw new InvalidOperationException("Buffer stream cannot be retrieved since buffering is disabled.");
        }
        return new AspNetCorePlus_CachedResponseBody(_segmentWriteStream.GetSegments(), _segmentWriteStream.Length);
    }
    ...
    public override IAsyncResult BeginWrite(byte[] buffer, int offset, int count, AsyncCallback? callback, object? state)
        => AspNetCorePlus_TaskToApm.Begin(WriteAsync(buffer, offset, count, CancellationToken.None), callback, state);

    public override void EndWrite(IAsyncResult asyncResult)
        => AspNetCorePlus_TaskToApm.End(asyncResult);
}
